...
router.post("/", upload, (req, res) => {
  let dirPath = path.join(__dirname, '..', 'resource', 'image', path.basename(req.file.filename, path.extname(req.file.filename)))
  ...
  if(!fs.existsSync(dirPath)) {
    ...
    pdf.convert(path.join(__dirname, '..', 'resource', 'uploads', path.basename(req.file.originalname)), opts)
      .then(() => {
        let pathArray = fs.readdirSync(dirPath).map((filePath) =>
            path.join(
              "image",
