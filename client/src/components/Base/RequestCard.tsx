...
const RequestCard = memo(
  ({ page, active, data, setQueryParam }: RequestCardProps) => {
    const { id, title, status, priority, type, deadline, author } = data;
    const overdue =
      deadline && data.status === 'Pending'
          ...
          <Flex direction="column" py={2} px={2} w="full" className="Flex">
            ...
            <HStack spacing={1} mt={1}>
              <Text
                ...
                onClick={() =>
                  setQueryParam &&
                  setQueryParam('createdBy', author.id.toString(), author.name)
                }
              >
                {author.name}
              </Text>
            ...
            <HStack spacing={2} mt={2}>
              <StatusTag
                status={status}
                onClick={() =>
                  setQueryParam && setQueryParam('status', status, status)
                }
              />
              <TypeTag
                onClick={() =>
                  setQueryParam && setQueryParam('type', type, type)
                }
                type={type}
              />
              {priority === 'Urgent' && status === 'Pending' && (
                <UrgentTag
                  onClick={() =>
                    setQueryParam &&
                    setQueryParam('priority', priority, priority)
                  }
                />
              )}
              {overdue && <OverdueTag />}
