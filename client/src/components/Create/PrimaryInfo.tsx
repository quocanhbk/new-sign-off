...
interface PrimaryInfoProps
  extends Pick<
    IRequestInput,
    'title' | 'type' | 'procedure' | 'priority' | 'deadline' | 'relatedProjects'
  > {
...
const PrimaryInfo = ({
  ...
  const calculatedProcedureList = procedureList
    ? [...procedureList, { id: 0, title: 'Flexible' }]
    : [];
        ...
        <FormControl label={'Related Project'} error={errors.relatedProjects}>
          <Select
            ...
            value={projectList.find(
              (project) => project.id === relatedProjects[0]
            )}
            onSelect={(newValue) => setValue('relatedProjects', [newValue.id])}
          />
        </FormControl>
      </Flex>
    </Flex>
