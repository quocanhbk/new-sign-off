...
const ButtonChoose = styled.button`
    background-color: ${props => props.theme.color.fill.info};
...
`
function SavingDraftDocument({dataDraft,setdataDraft}) {
    ...
    const removeItem = (val) =>{
        setdataDraft(dataDraft.filter( item => item.id !== val.id))
        setModalRemove(false)
     }

    return (
        <StyleWrapper className="table-list-4">
        <Text>Please input infomation in order to store your approval document</Text>
        <DivInput>
        ...
        <Table>
            ...
            <Table.Body>
            {dataDraft.slice((activePage-1)*3, (activePage-1)*3 + 3).map((val,keys)=>{
                return (
                <Table.Row key={keys}>
                    ...
                    <Table.Cell textAlign="left">
                        <DivButton>
                            ...
                            <ButtonChoose>Overwrite</ButtonChoose>
                        </DivButton>
                    </Table.Cell>
                </Table.Row>
                );
            ...
            <Table.Footer>
                <Table.Cell colSpan="2" textAlign="center">
                    {"Total: " + dataDraft.length}
                </Table.Cell>
                <Table.Cell colSpan="99" textAlign="right">
                    <Pagination
                    totalPage={Math.ceil(dataDraft.length / 3)}
                    boundary={1}
                    sibling={1}
...
export default SavingDraftDocument;