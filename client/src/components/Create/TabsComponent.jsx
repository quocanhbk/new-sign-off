...
`
const TabsComponent = ({tabItems,dataForm,handleFile}) => {
    const [active, setActive] = useState(tabItems[0].id);
     ...
     return (
       ...
       <StyleTabs className="tabs">
         {tabItems.map(({ id, name_required }) =><TabItemComponent
            key={id}
         ...
         {tabItems.map((val) => {
           ...
           return active === val.id ?
            ...
            <DropdownMenu label="Attach File">
                  ...
                  <FileUpload handleFile={(e) => handleFile(e,val.id)} name={val.name_required}/>
                  <FormSelect>
   ...
   const TabItemComponent = ({
    ...
    return (
      <TabItem className={!isActive ? 'tabitem' : 'tabitem tabitem--active'} onClick={onItemClicked}>
