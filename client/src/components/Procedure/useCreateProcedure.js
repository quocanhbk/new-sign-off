...
const useProcedure = (id) => {
    ...
    const { reset, render, setNotFound, setPercent } = useCustomLoader(
        !!id,
                ...
                getProcedureDetail(id, false, (v) => setPercent(v))
                    .then((procedure) => {
                        ...
                        dispatch({
                            ...
                            payload: {
                                ...procedure,
                                advisors: procedure.advisors.map(
                                    (advisor) => advisor.id
                                ),
                                approvers: procedure.approvers.map(
                                    (approver) => approver.id
                                ),
                                observators: procedure.observators.map(
                                    (observator) => observator.id
                                ),
                            },
                        })
                    })
    ...
    const removeForm = (checkItemId, formId) => {
        ...
        set("checklist", [
            ...
            {
                ...checkItemObj,
                defaultForms: checkItemObj.defaultForms.filter(
                    (form) => form.id !== formId
                ),
            },
            ...checklist.slice(checkItemIndex + 1, checklist.length),
