...
const useProcedure = id => {
    const [{ title, description, advisors, approvers, observators, tags, isActive, checklist }, dispatch] = useReducer(
        reducer,
        initState
    )
    ...
    useEffect(() => {
        if (id) {
            const fetchDetail = async () => {
                getProcedureDetail(id, v => setPercent(v))
                    .then(procedure => {
                        setPercent(100)
                        dispatch({
                            type: "INIT",
                            payload: {
                                ...procedure,
                                advisors: procedure.advisors.map(advisor => advisor.id),
                                approvers: procedure.approvers.map(approver => approver.id),
                                observators: procedure.observators.map(observator => observator.id),
                            },
                        })
                    })
    ...
    const submitProcedure = async () => {
        reset()
        if (id) {
            let newId = await updateProcedure(
                id,
                {
                    title,
                    description,
            ...
            let procedureId = await postProcedure(
                {
                    title,
                    description,
                    advisors,
    ...
    const toggleAdding = checkItemId => {
        ...
        set("checklist", [
            ...checklist.slice(0, checkitemIndex),
            {
                ...checklist[checkitemIndex],
                adding: !checklist[checkitemIndex].adding,
            },
            ...checklist.slice(checkitemIndex + 1, checklist.length),
        ])
    }
    const toggleLoading = checkItemId => {
        ...
        set("checklist", [
            ...
            {
                ...checklist[checkitemIndex],
                loading: !checklist[checkitemIndex].loading,
            },
            ...checklist.slice(checkitemIndex + 1, checklist.length),
        ])
    }
    const addForm = async (checkItemId, formId, callback: CallbackFunction = () => {}) => {
        ...
        setTimeout(
            () =>
                set("checklist", [
                    ...
                    {
                        ...checkItemObj,
                        defaultForms: [formDetail, ...checkItemObj.defaultForms],
                        adding: false,
                        loading: false,
                    },
                    ...checklist.slice(checkItemIndex + 1, checklist.length),
    ...
    const removeForm = (checkItemId, formId) => {
        ...
        set("checklist", [
            ...
            {
                ...checkItemObj,
                defaultForms: checkItemObj.defaultForms.filter(form => form.id !== formId),
            },
            ...checklist.slice(checkItemIndex + 1, checklist.length),
        ])
    }
