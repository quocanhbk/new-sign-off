...
import { approveRequest, cancelRequest, deleteRequest, getLastSignRequest, IRequest, remindApprove } from "api/request"
...
const DisplayContent = ({ id, mode }: DisplayContentProps) => {
    ...
    const [logs, setLogs] = useState<IRequest["logs"]>([])
    // used for storing viewing attachment
    const [editingAttachment, setEditingAttachment] = useState<{ id: Id; type: string } | null>(null)
    ...
    return (
        <Container className="container">
            <AbsoluteModal
                ...
                fixed
                overflow="overlay"
                height={device === "PC" ? "80%" : "100%"}
                width={device === "PC" ? "70%" : "100%"}
                maxWidth="880px"
            >
                {editingAttachment && (
                    <FormPopup
                        attachment={(editingAttachment.type === "approval"
                            ? request!.approvalAttachments
                            : request!.referenceAttachments
