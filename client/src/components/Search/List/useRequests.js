...
const useRequests = (mode) => {
    const RANGE = 10
    const { query, queryString, queryTags, onChangeTitleSearch, setQuery } =
        useRequestQuery()

    const { render, setNotFound, setPercent } = useCustomLoader(
        true,
    ...
    const getRequestsWithRange = async ({ pageParam = 0 }) => {
        let requests = await getRequests(
            queryString +
                `&start=${pageParam}&end=${pageParam + RANGE}&sign=${
                    mode === "sign"
                }`,
            setPercent
        )
