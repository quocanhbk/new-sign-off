...
export const getDarker = (hex, shade = 20) => {
    ...
    const shadeClr = Math.round((255 * shade) / 100)
    const newClr = clr.map(x => Math.round(Math.max(x - shadeClr, 0)))
...
export const getLighter = (hex, shade = 20) => {
    ...
    let shadeClr = Math.round((255 * shade) / 100)
    let newClr = clr.map(x => Math.round(Math.min(x + shadeClr, 255)))
    return "RGB(" + newClr[0] + ", " + newClr[1] + ", " + newClr[2] + ")"
}
